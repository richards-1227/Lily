...
int main(int argc, char *argv[]) {
 ...
 while(inf){
    ...
    int noSeq{};
    noSeq=static_cast<int>(seqArray2.size());
        ...
        for(int j=(i+1); j<(noSeq-2); ++j){
            for(int k=(j+1); k<(noSeq-1); ++k){
                for(int l=(k+1); l<noSeq; ++l){
                    for(int m=0; m<seqArray2[i].size(); ++m){
                        for(int n=0; n<seqArray2[j].size(); ++n){
                            for(int o=0; o<seqArray2[k].size(); ++o){
                                for(int p=0; p<seqArray2[l].size(); ++p){
                    outputSum output1{i, j, k, l, m, n, o, p, 0, 0, 0};
    ...
    [&](tbb::blocked_range<int> v)
    {
          for(int taxa4=v.begin(); taxa4<v.end(); ++taxa4){
              tbb::parallel_for( tbb::blocked_range<int>(0,sampleCount[taxa1]),
                                [&](tbb::blocked_range<int> a)
                                {
                                    for (int obs1=a.begin(); obs1<a.end(); ++obs1){
                  tbb::parallel_for( tbb::blocked_range<int>(0,sampleCount[taxa2]),
                                    [&](tbb::blocked_range<int> b)
                                    {
                                        for (int obs2=b.begin(); obs2<b.end(); ++obs2){
                         tbb::parallel_for( tbb::blocked_range<int>(0,sampleCount[taxa3]),
                      [&](tbb::blocked_range<int> c)
                      {
                          for (int obs3=c.begin(); obs3<c.end(); ++obs3){
                                 tbb::parallel_for( tbb::blocked_range<int>(0,sampleCount[taxa4]),
                              [&](tbb::blocked_range<int> d)
                              {
                                  for (int obs4=d.begin(); obs4<d.end(); ++obs4){
                
    std::vector<std::string> seqArray3{
        seqArray2[taxa1][obs1], seqArray2[taxa2][obs2], seqArray2[taxa3][obs3]
               , seqArray2[taxa4][obs4]
                };
         
              double beta{};
              
              std::vector<int> pair1{pairCounts(seqArray2[taxa1][obs1], seqArray2[taxa2][obs2])};
              std::vector<int> pair2{pairCounts(seqArray2[taxa1][obs1], seqArray2[taxa3][obs3])};
              std::vector<int> pair3{pairCounts(seqArray2[taxa1][obs1], seqArray2[taxa4][obs4])};
              std::vector<int> pair4{pairCounts(seqArray2[taxa2][obs2], seqArray2[taxa3][obs3])};
              std::vector<int> pair5{pairCounts(seqArray2[taxa2][obs2], seqArray2[taxa4][obs4])};
              std::vector<int> pair6{pairCounts(seqArray2[taxa3][obs3], seqArray2[taxa4][obs4])};
              ...
              std::vector<int> countVec{generateCounts(seqArray3)};
   ...
   std::vector<double> finalProbVec{0,0,0};
       ...
       if( (phat1==0 && phat2==0 && phat3==0 && phat4==0 && phat5==0 && phat6==0)
          || phat1 > 0.74 || phat2 > 0.74 || phat3 > 0.74
          || phat4 > 0.74  || phat5 > 0.74  || phat6 > 0.74){
           finalProbVec={1.0/3, 1.0/3, 1.0/3};
       }
